...
public class ModelB {
    ...
    public ModelB(int previousYear) {
    ...
    public double performModel(String stat, PlayerGame game, double avgRec, double line) throws IOException {
        ...
        double confidence = statValue-line;
        if ( confidence > 0 && statValue > 5.5 )return round(statValue, 4);
        else if ( confidence < 0 && statValue < 5.5 ) return round(statValue, 4);
        else return -1.0000;
    }
